...
function toggleWebcamMenu() {
  $('.body-webcam header').toggleClass('fade');
  $('#brand').toggleClass('fade-header');

  if ($('#hide-btn-webcam').children('i').hasClass('down')) {
    $('#hide-btn-webcam').children('i').removeClass('down');
  } else {
    $('#hide-btn-webcam').children('i').addClass('down');
  }

  $('#hide-btn-webcam').children('h6').toggleClass('active');

  if ($('#hide-btn-webcam').find('h6').hasClass('active')) {
    $('.btns').addClass('topOffset');
  }

  if ($('#hide-btn-webcam').find('h6:last').hasClass('active')) {
    $('.btns').removeClass('topOffset');
  }
}
...
$(document).ready(function () {
  ...
  $(window).on('load resize', function () {
    ...
    var parallax_enable = !isMobileScreen();
  ...
  function runParallax() {
    ...
    for (var i = 0; i < sections_num; i++) {
      // $.each(content, function () {
      // 	$(this).css('top', 0)
      // });
      // $(section[i]).css('max-height', '');
      var sectionHeight = $(section[i]).outerHeight();
      var temp = 300 + sectionHeight - (scrollSpeed - 1) * sectionHeight / 10;
      var maxHeigth = temp < scr_h + 300 ? scr_h + 300 : temp; // $(section[i]).css('max-height', maxHeigth + 'px');
  ...
  function stopParallax() {
    page.removeClass('parallax'); //init = false;
  ...
  function scrollParallax() {
    ...
    for (var i = 0; i < section.length; i++) {
      var drop = scrollPosition - sectionTop[i];
      var shift = -(scrollSpeed - 1) * drop / 10; //$(content[i]).css('top', shift + 'px')
  ...
  var top_offset = $('#brand').outerHeight();
  var anchors = $('[paginationAnchor]');
  var anchors_top = [];
  var current_anchor_container = $('#current-page');
  var current_anchor;
  var total_anchors_container = $('#total-pages');
  var total_anchors = anchors.length;
  var page_up = $('#page-up');
  var page_down = $('#page-down');
...
!function (e) {
  ...
  e.modal = w;
}(Custombox || (Custombox = {})); //popup

$(document).ready(function () {
  // array of popup id`s. for add popup just add id in array. for open popup add attribute data-title=${namePopup} to btn or etc.
  var popupMapping = ['disclaimer', 'privacy', 'contact', 'enquiry', 'recommendation'];

  function closeModal() {
    Custombox.modal.close();
  }

  popupMapping.forEach(function (namePopup) {
    var popup = new Custombox.modal({
      content: {
        effect: 'fadein',
        target: '#' + namePopup,
        delay: 50
  ...
  $('.close-button').click(closeModal); //fix to (1) don`t close popup at click on itself (2)
...
$(document).ready(function () {
  var panels = $.makeArray($('.panel:not(.image-panel, .p-0, .content-main .panel)'));
...
$(document).ready(function () {
  ...
  var scrollTriggerFromBottom = 1800; // px

  var toogleButton = function toogleButton() {
...
(function (factory) {
  ...
  $.fn.niceScroll = function (wrapper, _opt) {
    if (_opt === undefined && _typeof2(wrapper) == "object" && !("jquery" in wrapper)) {
      _opt = wrapper;
      wrapper = false;
    }
...
$(document).ready(function () {
  ...
  $('#hide-btn-webcam').on('click', function () {
    toggleWebcamMenu();
  });
...
$(document).ready(function () {
  var x = [{
    ...
    label: "APART'HOTEL, Hotel, Type H2"
  }, {
    url_preview: "113408_01_v2.jpg",
    url_panorama: "113408_01_v2",
    label: "APART'HOTEL, Hotel, Type H1"
  }, {
    url_preview: "114185.jpg",
    url_panorama: "114185",
    label: "APART'HOTEL, Studio"
  }, {
    url_preview: "113391.jpg",
    url_panorama: "113391",
    label: "APART'HOTEL, 1 Bedroom"
  }, {
    url_preview: "83782_06_v3.jpg",
    url_panorama: "83782_01_v3",
  ...
  for (var item in x) {
    ...
    $('#panorama-list').append('<div class="panel ' + active + '" data-url="' + getUrl.protocol + '//' + getUrl.host + "/media/panorama/" + x[item].url_panorama + '/ ">' + '<div class="panel-body">' + '<img src="/media/preview/' + x[item].url_preview + '" alt="" >' + '<h6>' + x[item].label + '</h6>' + '</div>' + '</div>');
  }
...
$(document).ready(function () {
  ...
  $('#surname, #email').keyup(function (e) {
    $('#send').html('Send');
    var test_firstname = $('#enquiry').find("input[placeholder='First name*']").val();
    var test_surname = $('#enquiry').find("input[placeholder='Surname*']").val();
    var test_email = $('#enquiry').find("input[placeholder='Email*']").val();
...
$(document).ready(function () {
  ...
  $('#email-newsletter').keyup(function (e) {
...
$(document).ready(function () {
  ...
  $('#recemail, #sendemail').keyup(function (e) {
    $('#sendrec').html('Send');
    var firstname = $('#sendname').val();
    var rec_email = $('#recemail').val();
